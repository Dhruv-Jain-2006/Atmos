...
export async function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-edge bg-void/85 backdrop-blur-md">
      <div className="mx-auto max-w-[1400px] px-4 py-2.5 sm:px-5 sm:py-3">
        {/* Row 1: logo + nav + status (desktop), logo + nav (mobile) */}
        <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-2.5 sm:gap-x-8">
          ...
          {/* Desktop: status inline */}
          <div className="hidden items-center gap-3 sm:flex" title={status.detail}>
            <Link
        ...
        {/* Row 2: GitHub + status on mobile only */}
        <div className="mt-2 flex items-center gap-3 sm:hidden" title={status.detail}>
          <Link
            href="https://github.com/Dhruv-Jain-2006/Atmos"
            target="_blank"
            rel="noopener noreferrer"
